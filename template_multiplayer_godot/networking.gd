...
func conectado_ao_servidor():
	...
	rpc("registrar_jogador", id, nome_jogador, r, g, b) #chamada remota rpc(função, parametros)
	pass
...
@rpc("any_peer")
func registrar_jogador(id, nome, R, G, B):
	if multiplayer.is_server():
		for i in range(jogadores.size()):
			rpc_id(id, "registrar_jogador", jogadores[i][0], jogadores[i][1], jogadores[i][2], jogadores[i][3], jogadores[i][4])
		rpc("registrar_jogador", id, nome, R, G, B)
	jogadores.append([id, nome, R, G, B])
...
func criar_servidor():
	...
	if ip.begins_with("192"):
		...
		id = multiplayer.multiplayer_peer.get_unique_id()
		registrar_jogador(id, nome_jogador, r, g, b)
	else:
		resetar_conexao()
	pass
...
func atualizar_nome(novo_nome, novo_r, novo_g, novo_b):
